...
class Network:
    ...
    def updates(self, optimization_scheme, loss, learning_rate):
        ...
        if optimization_scheme == 'rmsprop':
            return lasagne.updates.sgd(loss, params, learning_rate=learning_rate)
        if optimization_scheme == 'adam':
            return lasagne.updates.adam(loss, params, learning_rate=learning_rate, beta1=0.9, beta2=0.999, epsilon=1e-08)
        if optimization_scheme == 'adadelta':
            return lasagne.updates.adadelta(loss, params, learning_rate=learning_rate, rho=0.95, epsilon=1e-06)
        else:
            pass
...
#        for channel in range(params.shape[1]):
        ...
        filter_matrix.save("filters/filter_"+str(id)+"_train.png")
    ...
    def train(self, max_epochs, batch_size):
        ...
        #training with adam optimization scheme
        train_function = theano.function([input_data, labels], loss, updates=self.updates('adadelta', loss, 1.0))
