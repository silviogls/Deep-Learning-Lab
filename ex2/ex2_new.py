...
class Network:
    ...
    def build_network(self, input_data):
        ...
        conv_layer_1 = lasagne.layers.Conv2DLayer(network, num_filters=64, pad = 'same', filter_size=(5, 5), nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.GlorotUniform())
        network = lasagne.layers.MaxPool2DLayer(conv_layer_1, pool_size=(2, 2))
        conv_layer_2 = lasagne.layers.Conv2DLayer(network, num_filters=64, pad = 'same', filter_size=(5, 5), nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.GlorotUniform())
    ...
    def train(self, max_epochs, batch_size):
        ...
        train_function = theano.function([input_data, labels], loss, updates=lasagne.updates.sgd(loss, params, learning_rate=0.3))
        ...
        for epoch in range(max_epochs):
            ...
            for input_batch, labels_batch in self.batches(self.train_images[:50000,:,:,:], self.train_labels[:,20], batch_size):
            ...
            for input_batch, labels_batch in self.batches(self.val_images[:10000,:,:,:], self.val_labels[:,20], batch_size):
