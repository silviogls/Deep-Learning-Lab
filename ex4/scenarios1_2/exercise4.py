...
def run_then_return_val_loss(nepochs, hyperparameters, noiselevel, xshift=0.2):
...
def scenario(scenario, xshift = 0.2):
    iscenario = scenario
    if (iscenario == 1):
        ...
        for nepochs in [1, 3, 9, 27, 81]:   # with different resolution / number of epochs
            for i in range(0, nx):                  # for different hyperparameter values
                ...
                y_i = run_then_return_val_loss(nepochs, x_i, noiselevel, xshift)        # we evaluate them
        ...
        for irun in range(0, 100):
            ...
            for i in range(nrandom):
                ...
                y_i = run_then_return_val_loss(81, x_i, noiselevel, xshift)
                ...
                y_best_observed_denoised = run_then_return_val_loss(81, x_best_observed, 1e-10, xshift=xshift)
                y_i = run_then_return_val_loss(81, x_best_observed, y_best_observed_denoised, xshift=xshift)
                stat_file.write("{}\t{:.15g}\n".format(i, y_i))
            stat_file.close()
            ...
            for s in reversed(range(s_max+1)):
                ...
                for i in range(s+1):
                    ...
                    val_losses = [ run_then_return_val_loss(nepochs=r_i, hyperparameters=t,
                                                            noiselevel=noiselevel, xshift=xshift) for t in T ]
                    ...
                    if (len(T)):
                        f_recommendation = run_then_return_val_loss(81, x_best_observed, 1e-10, xshift=xshift) # 81 epochs and 1e-10 ~= zero noise
